<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl leading-tight">Admin — Categorias</h2>
  </x-slot>

  <div class="py-6 max-w-6xl mx-auto">
    <div class="mb-4">
      <a href="{{ route('admin.categories.create') }}" class="px-3 py-2 bg-blue-600 text-white rounded">Nova Categoria</a>
    ...
    @php
      ...
      $pkCol    = $pk ?? 'id';
      $nameCol  = $pick(['category','name','title']);
      $slugCol  = $pick(['slug','descricao','description']);
    @endphp>
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-b bg-gray-50">
            ...
            <th class="p-3 text-left">Slug/Descrição</th>
        ...
        <tbody>
          @forelse($list as $row)
            ...
            <tr class="border-b">
              ...
              <td class="p-3 space-x-2">
                <a href="{{ route('admin.categories.edit', $id) }}" class="px-2 py-1 bg-yellow-500 text-white rounded">Editar</a>
                <form action="{{ route('admin.categories.destroy', $id) }}" method="POST" class="inline" onsubmit="return confirm('Remover categoria?');">
                  @csrf @method('DELETE')
          ...
          @empty
            <tr><td class="p-3" colspan="4">Nenhuma categoria encontrada.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
