  ...
  <section>
    ...
    @forelse($pending as $r)
      <article class="mb-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="min-w-0">
            <div class="font-medium text-slate-800 truncate">
              Biz #{{ $r->biz_id }} â€” {{ $r->business->business_name ?? 'N/A' }}
            </div>
            <div class="text-xs text-slate-500">
              Por: {{ $r->user?->name ?? 'Visitante' }} â€” <x-stars :value="$r->rating" size="sm" />
            </div>
          </div>
        ...
        @if($r->title)<div class="mt-2 font-semibold text-slate-800">{{ $r->title }}</div>@endif
        @if($r->body)<p class="mt-1 text-slate-700">{{ $r->body }}</p>@endif
      </article>
    @empty
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex gap-2">
            ...
            <form method="POST" action="{{ route('admin.reviews.destroy', $r) }}">
              @csrf @method('DELETE')
              <button class="rounded-lg bg-red-700 px-4 py-2 text-white hover:bg-red-800"
                      onclick="return confirm('Excluir review?')">Excluir</button>
            </form>
          </div>
        </div>
        @if($r->title)<div class="mt-2 font-semibold text-slate-800">{{ $r->title }}</div>@endif
